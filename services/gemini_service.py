...
def normalize_gemini_model(model_name: str) -> str:
    ...
    if normalized_model == HINT_GEMINI_MODEL or normalized_model in GEMINI_MODEL_OPTIONS:
...
def build_hint_prompt(topic: str, difficulty: str, question: Question) -> str:
    normalized_topic = topic.strip() or "general knowledge"
    difficulty_key = difficulty if difficulty in DIFFICULTY_DETAILS else "medium"
    difficulty_details = DIFFICULTY_DETAILS[difficulty_key]
    difficulty_label = difficulty_details["label"]
    ...
    return f"""
You are helping a student answer a multiple-choice quiz question.
...
Write exactly one short hint in 1 or 2 sentences.
Rules:
- Help the student reason toward the answer without revealing it.
- Do not quote the correct option verbatim.
- Do not mention option letters.
- Keep the hint under 35 words.
- No markdown, bullets, or extra commentary.
""".strip()
...
def generate_question_hint(topic: str, difficulty: str, question: Question) -> str:
    client = get_gemini_client()
    prompt = build_hint_prompt(topic=topic, difficulty=difficulty, question=question)
    response = _generate_content_with_retries(client, prompt, model_name=HINT_GEMINI_MODEL)
    response_text = getattr(response, "text", None)
    if not response_text:
