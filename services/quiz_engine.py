...
def ensure_hint_slots(session: QuizSession) -> None:
    if len(session.hints) < len(session.questions):
        session.hints.extend([None] * (len(session.questions) - len(session.hints)))
    elif len(session.hints) > len(session.questions):
        del session.hints[len(session.questions) :]


def sync_current_question_state(session: QuizSession) -> None:
    ensure_answer_slots(session)
    ensure_hint_slots(session)
...
def store_questions_in_session(
    ...
    if replace_existing:
        ...
        session.score = 0
        session.answers = [None] * len(session.questions)
        session.hints = [None] * len(session.questions)
        session.selected_option = None
        session.submitted = False
    else:
...
def store_question_hint(session: QuizSession, question_index: int, hint_text: str) -> None:
    ensure_hint_slots(session)
    if question_index < 0 or question_index >= len(session.questions):
        raise IndexError("Question index is out of range for hint storage.")
    session.hints[question_index] = hint_text.strip()
