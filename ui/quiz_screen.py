...
def render_quiz_ui() -> None:
    ...
    hint_feedback = st.session_state.pop("hint_feedback", None)
    ...
    current_hint = None
    if st.session_state.question_index < len(st.session_state.hints):
        current_hint = st.session_state.hints[st.session_state.question_index]
    ...
    if hint_feedback and hint_feedback["type"] == "error":
        st.error(hint_feedback["message"])
    ...
    if current_hint:
        st.text_area(
            ...
            height=90,
            disabled=True,
            key=f"hint_box_{st.session_state.question_index}",
        )
    ...
    with st.container(key="quiz-nav"):
        ...
        with hint_col:
            hint_button_label = "Generating.." if st.session_state.get("is_generating_hint") else "Hint"
            if st.button(
                hint_button_label,
                key="quiz_hint",
                disabled=not key_is_configured or bool(st.session_state.get("is_generating_hint")),
            ):
                queue_hint_generation()
                st.rerun()
